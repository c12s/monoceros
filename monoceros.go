...
func (m *Monoceros) promote(network *TreeOverlay) {
	...
	tree := plumtree.TreeMetadata{
		Id:    fmt.Sprintf("%s_%d", network.ID, m.config.NodeID),
		Score: float64(m.config.NodeID),
	}
...
func (m *Monoceros) initAggregation(network *TreeOverlay) {
	for range network.aggregate {
		...
		err = network.plumtree.Broadcast(network.local.Id, "AGGREGATION_REQ", msgBytes)
		if err != nil {
...
func (m *Monoceros) onMessage(tree plumtree.TreeMetadata, msgType string, msg []byte, sender data.Node) bool {
	...
	if msgType == "AGGREGATION_RESP" {
		...
		go m.onAggregationResp(network, tree, resp, sender)
		return false
	}
	if msgType == "RANK_LIST" {
		list := RankList{}
...
func (m *Monoceros) onAggregationReq(network *TreeOverlay, tree plumtree.TreeMetadata, req AggregationReq) bool {
	if network.local != nil && network.local.Id != tree.Id && tree.HasHigherScore(*network.local) {
...
func (m *Monoceros) processAggregationReq(network *TreeOverlay, tree plumtree.TreeMetadata, req AggregationReq) {
	...
	if len(receivers) == 0 {
		m.completeAggregationReq(network, tree, req.Timestamp, localVal, localScores)
	} else {
...
func (m *Monoceros) completeAggregationReq(network *TreeOverlay, tree plumtree.TreeMetadata, timestamp int64, value int64, scores map[int64]float64) {
	...
	if network.plumtree.HasParent(tree.Id) {
		...
		if err != nil {
			...
			err = network.plumtree.Broadcast(tree.Id, "RANK_LIST", list)
			if err != nil {
				m.logger.Println("error sending rank list", err)
			} else {
				m.logger.Println("sent rank list", list)
			}
		}
		// eval rules
...
func GetNodeRank(nodeID int64, scores map[int64]float64) int64 {
