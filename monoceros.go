	...
	clearActive := func(network *TreeOverlay) {
		for range time.NewTicker(time.Second).C {
			...
			for _, aar := range network.activeRequests {
				...
				// todo: ??
				if len(IntersectPeers(children, aar.WaitingFor)) == 0 || aar.Timestamp+(1*m.config.Aggregation.TAggSec*1000000000) < time.Now().UnixNano() {
...
func (m *Monoceros) onGlobalMsg(msgBytes []byte, from transport.Conn) bool {
	...
	if msgType == RRUPDATE_MSG_TYPE {
		...
		roots, ok := m.Roots[msg.Region]
		if !ok {
			roots = make([]RootInfo, 0)
		}
		if msg.Joined {
			if !slices.ContainsFunc(roots, func(r RootInfo) bool {
				return r.NodeInfo.ID == msg.NodeInfo.ID
			}) {
				roots = append(roots, msg)
			}
		} else {
			roots = slices.DeleteFunc(roots, func(r RootInfo) bool {
				return r.NodeInfo.ID == msg.NodeInfo.ID
			})
		}
		m.Roots[msg.Region] = roots
		return true
...
func (m *Monoceros) onAggregationReq(network *TreeOverlay, tree plumtree.TreeMetadata, req AggregationReq, sender transport.Conn) bool {
	...
	cancel := false
	if ((!m.HasHigherScore(tree.NodeID(), tree.Score) && tree.NodeID() != m.config.NodeID) || slices.ContainsFunc(network.activeRequests, func(r *ActiveAggregationReq) bool {
...
func (m *Monoceros) contactRRNNode() {
	contactedPeer := false
	for region, roots := range m.Roots {
